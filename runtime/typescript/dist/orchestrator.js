...
const ZERO_METRICS = { latency_ms: 0, input_tokens: 0, output_tokens: 0 };
// ── Public API (called by generated workflow stubs) ──
export async function orchestrate(workflowName, input) {
...
async function executeWorkflow(workflow, input, agents) {
    const requestId = crypto.randomUUID();
    const startedAt = new Date().toISOString();
    const stepResults = [];
    const context = { input, steps: {} };
    let shortCircuited = false;
    let shortCircuitDefaults;
    try {
        for (const entry of workflow.steps) {
            if (isParallelGroup(entry)) {
                ...
                const results = await Promise.all(entry.parallel.map((step) => executeStepWithRetry(step, context, agents)));
        ...
        const envelope = buildEnvelope(workflow, requestId, startedAt, stepResults, result, shortCircuited ? 'short_circuited' : 'success');
...
async function executeStepWithRetry(step, context, agents) {
    ...
    if (!agentDef) {
        ...
        throw new Error(`Agent not found: ${step.agent}. Available: ${[...agents.keys()].join(', ')}`);
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            log.info(`Step ${step.id}: success (attempt ${attempt})`, {
                ...
                output_type: result.output && typeof result.output === 'object'
...
function makeErrorResult(step, err, attempts, usedFallback) {
    return {
        id: step.id,
        agent: step.agent,
        status: 'error',
...
function evaluateCondition(condition, output) {
    try {
...
function fillSkipped(step, defaults, context, results) {
...
function buildEnvelope(workflow, requestId, startedAt, steps, result, status, error) {
    let totalLatency = 0;
    let totalInput = 0;
    let totalOutput = 0;
    let executed = 0;
