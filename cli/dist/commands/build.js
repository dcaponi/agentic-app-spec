...
import { findProjectRoot, loadYaml, toCamelCase, toPascalCase, toSnakeCase, toGoPublic, yamlTypeToTS, yamlTypeToPython, yamlTypeToRuby, yamlTypeToGo, } from '../utils.js';
function extractFields(input) {
    if (!input || typeof input !== 'object')
        return [];
    return Object.entries(input).map(([name, spec]) => ({
        name,
        yamlType: spec?.type ?? 'string',
    }));
}
...
function genTypeScriptAgent(agent) {
...
function genTypeScriptWorkflow(wf) {
...
// ── Go generator ────────────────────────────────────────────────────────────
function genGoAgent(agent) {
    const pascal = toGoPublic(agent.id);
    const fieldLines = agent.fields
        .map((f) => `    ${toGoPublic(f.name)} ${yamlTypeToGo(f.yamlType)} \`json:"${toSnakeCase(f.name)}"\``)
        .join('\n');
    return `// @generated from ${agent.yamlPath} — do not edit
package agents
...
export function runBuild(opts) {
    ...
    const lang = opts.lang ?? configLang;
    const outdir = opts.outdir ?? configOutdir;
    const validLangs = ['typescript', 'python', 'ruby', 'go'];
